...
# scoring = ['balanced_accuracy', 'f1_weighted','average_precision','recall']
                     
# Create a based model
rf = SVC()
# Instantiate the grid search model
grid_search = RandomizedSearchCV(estimator = rf, param_distributions = param_grid, random_state=42,
                          cv = 5, n_jobs = 20, verbose = 2,scoring = scoring,n_iter = 100,refit= 'accuracy')
...
# clf = SVC(kernel='poly', degree=2, C=5.0,gamma = 20)
# clf.fit(X_train, y_train.values.ravel())
# plt.plot(X_test.index,y_test, c='b', label='Real values')
# plt.plot(X_test.index,clf.predict(X_test), c='g', label='Prediction Values')
# plt.legend()
# plt.show()
