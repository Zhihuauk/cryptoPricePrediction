# -*- coding: utf-8 -*-
"""
Created on Fri Mar 11 06:45:36 2022
...
# load the dataset
df = pd.read_csv(r'G:\MScProject\data\norm_df.csv')
print(df.dtypes)
#--------------------------------data proprecessing---------------------------#


df['time_real'] = pd.to_datetime(df['time_real'])
print(df.dtypes)

df.set_index('time_real', inplace=True) 

plt.plot(df['time_real'])
#--------------------------------data proprecessing---------------------------#
...
for col in df.columns:
    ...
    col = adf_test.should_diff(df[col])
...
for col in df.columns:
    adf, pvalue, usedlag_, nobs_, critical_values_, icbest_ = adfuller(df[col])
    print(col,"pvalue is ", pvalue, " if above 0.05, data is not stationary")
...
arima_model = auto_arima(df['price_usd_close'], start_p = 1, d=1, start_q = 1, 
